...
create table if not exists public.companies (
  ...
  scheduling_timezone text not null default 'America/Chicago',
  scheduling_workdays smallint[] not null default array[1, 2, 3, 4, 5]::smallint[],
  scheduling_workday_start time not null default '08:00',
  scheduling_workday_end time not null default '17:00',
...
alter table public.companies add column if not exists scheduling_timezone text not null default 'America/Chicago';
alter table public.companies add column if not exists scheduling_workdays smallint[] not null default array[1, 2, 3, 4, 5]::smallint[];
alter table public.companies add column if not exists scheduling_workday_start time not null default '08:00';
alter table public.companies add column if not exists scheduling_workday_end time not null default '17:00';
alter table public.companies add column if not exists scheduling_buffer_minutes integer not null default 30;
...
begin
  ...
  if target_job_id is not null then
    select scheduled_start, recurrence_frequency, recurrence_interval
    into old_scheduled_start, old_recurrence_frequency, old_recurrence_interval
    from public.jobs
    where id = target_job_id
      and company_id = target_company_id;
  end if;

  saved_job := public.save_job_record(
    target_company_id,
    target_job_id,
    input_customer_name,
    ...
    input_job_industry,
    input_job_name,
    input_job_service_address,
    input_job_status,
    input_job_material_status,
    input_job_projected_date,
    input_job_invoice_url,
    input_job_next_action,
    input_job_internal_notes,
    input_job_custom_values
  );
  ...
  if target_job_id is not null and (
    old_scheduled_start is distinct from input_job_scheduled_start
    or old_recurrence_frequency is distinct from coalesce(input_job_recurrence_frequency, 'none')
    or old_recurrence_interval is distinct from coalesce(input_job_recurrence_interval, 1)
  ) then
...
create policy "Members can manage schedule exceptions"
...
-- Correct the promo argument/column ambiguity in the subscription-aware bootstrap.
create or replace function public.bootstrap_company(company_name text, promo_code text default null)
returns public.companies
language plpgsql
security definer
...
-- Subscription access, fourteen-day trials, and read-only enforcement.
