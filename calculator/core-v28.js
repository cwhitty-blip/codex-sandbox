...
function open(d){if(projects.some(x=>x.id===d.id))return project(d);if(d.id==='notes')return notes();if(d.id==='clock')return clock();if(d.id==='browser')return browser();if(d.id==='settings')return settings();return simple(d.name)}
function renderPhone(){const pages=$('#phonePages'),dots=$('#pageDots'),dock=$('#phoneDock');if(!pages||!dots||!dock)return;pages.innerHTML='';dots.innerHTML='';dock.innerHTML='';const all=[...projects,...builtins.filter(x=>!dockIds.includes(x.id))],groups=[];for(let i=0;i<all.length;i+=12)groups.push(all.slice(i,i+12));if(groups.length<2)groups.push([]);groups.forEach((g,n)=>{const page=document.createElement('div');page.className='phone-page';const grid=document.createElement('div');grid.className='phone-grid';g.forEach(d=>grid.append(icon(d,()=>open(d))));page.append(grid);pages.append(page);const dot=document.createElement('span');dot.className='page-dot'+(n===0?' active':'');dots.append(dot)});dockIds.forEach(id=>{const d=builtins.find(x=>x.id===id);dock.append(icon(d,()=>open(d)))});window.dispatchEvent(new CustomEvent('calculator-home-rendered'))}
function time(){const e=$('#phoneTime');if(e)e.textContent=new Date().toLocaleTimeString([],{hour:'numeric',minute:'2-digit'}).replace(' ','')}
renderPhone();time();setInterval(time,30000);window.CalculatorCore={show,home,app,renderPhone,toast,lock};
})();
