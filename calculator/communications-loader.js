...
async function start(){try{await loadScript('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2',7000);await loadScript('./phone-system.js?v='+VERSION,4000);await loadScript('./message-launcher.js?v='+VERSION,4000);await loadScript('./phone-polish.js?v='+VERSION,4000);await loadScript('./phone-keypad-fix.js?v='+VERSION,2500);await loadScript('./call-ring-safe.js?v='+VERSION,2500);window.dispatchEvent(new CustomEvent('calculator-communications-ready'))}catch(err){console.warn('Calculator communications unavailable',err);window.__calculatorCommunicationsUnavailable=true}}
setTimeout(start,60);
})();