...
let edit=false,drag=null,hold=null,startX=0,startY=0;
...
function setEdit(on){edit=on;home.classList.toggle('home-editing',on);pages.querySelectorAll('.phone-app').forEach(el=>el.draggable=on);if(on)toast('Drag apps to move them')}
function nearestGrid(x){const grids=[...pages.querySelectorAll('.phone-grid')];return grids.find(g=>{const r=g.getBoundingClientRect();return x>=r.left&&x<=r.right})||grids[0]}
function moveAt(el,x,y){const grid=nearestGrid(x);if(!grid)return;const others=[...grid.children].filter(c=>c!==el);let before=null,best=Infinity;for(const o of others){const r=o.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,d=(cx-x)**2+(cy-y)**2;if(d<best){best=d;before=o}}if(before)grid.insertBefore(el,before);else grid.append(el)}
pages.addEventListener('pointerdown',e=>{const el=e.target.closest('.phone-app');if(!el)return;startX=e.clientX;startY=e.clientY;hold=setTimeout(()=>{drag=el;setEdit(true);el.classList.add('dragging-app')},520)});
pages.addEventListener('pointermove',e=>{if(hold&&Math.hypot(e.clientX-startX,e.clientY-startY)>12){clearTimeout(hold);hold=null}if(!drag)return;e.preventDefault();moveAt(drag,e.clientX,e.clientY)});
function end(){if(hold)clearTimeout(hold);hold=null;if(drag){drag.classList.remove('dragging-app');drag=null;saveOrder()}}
