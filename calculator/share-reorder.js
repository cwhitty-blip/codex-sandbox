...
function applyOrder(){
 if(applying)return;const order=savedOrder();if(!order.length)return;
 applying=true;
 try{
  const grids=[...pages.querySelectorAll('.phone-grid')];
  const byKey=new Map([...pages.querySelectorAll('.phone-grid>.phone-app')].map(el=>[keyOf(el),el]));
  order.sort((a,b)=>a.page-b.page||a.order-b.order).forEach(x=>{
   const el=byKey.get(x.key),grid=grids[x.page];if(!el||!grid)return;
   const targetIndex=Math.min(x.order,grid.children.length);
   const currentGrid=el.parentElement,currentIndex=currentGrid?[...currentGrid.children].indexOf(el):-1;
...
function scheduleSync(){if(scheduled||applying)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;const added=injectShare();if(!added)applyOrder()})}
...
function end(){if(hold)clearTimeout(hold);hold=null;stopEdge();if(drag){drag.classList.remove('dragging-app');drag=null;saveOrder()}setEdit(false)}
...
const obs=new MutationObserver(()=>{if(!applying)scheduleSync()});obs.observe(pages,{childList:true,subtree:true});
window.addEventListener('calculator-home-rendered',scheduleSync);
injectShare();applyOrder();
})();